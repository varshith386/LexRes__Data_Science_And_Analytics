...
# Genrates Rules
def read_dataset(main_text_dir, rules_dir):
    dataset = []
    for file in os.listdir(rules_dir):
        if file.endswith("_rules.txt"):
            file_id = file[:-10]  # Removing the '_rules.txt' suffix to get the base filename
            main_text_file = f"{file_id}.txt"
            rules_summary_file = file  # This is the same as the current file

            main_text_path = os.path.join(main_text_dir, main_text_file)
            rules_summary_path = os.path.join(rules_dir, rules_summary_file)

            print(f"Trying to match main text file: {main_text_path} with rules summary file: {rules_summary_path}")

            if os.path.exists(main_text_path):  # Ensure the main text file exists
                with open(main_text_path, 'r') as f:
                    main_text = f.read()

                with open(rules_summary_path, 'r') as f:
                    rules_summary = f.read()
...
rules_dir = r'FIRAC SUMMARIES\RULES'  # Change this to your rules directory path
dataset = read_dataset(main_text_dir, rules_dir)
...
def prepare_training_data(dataset):
    ...
    for data in dataset:
        ...
        target_texts.append(f"summarize: {rules_summary} [rule]")
...
training_args = TrainingArguments(
    output_dir="./results_rules",  # Update output directory if needed
...
# Function to generate rules summary
def generate_rule_summary(model, tokenizer, text):
    input_text = text
...
# Example usage
def summarize_rules(file_path):
    with open(file_path, 'r') as file:
        text = file.read()
    return generate_rule_summary(model, tokenizer, text)

example_file_path = r'dataset\IN-Ext\judgement\1953_L_1.txt'  # Change this to the path of your example text file
rules_summary = summarize_rules(example_file_path)
print(rules_summary)
